...
def get_scoring_function(scoring=None):
    if scoring == "completion_error":
        ...
        def f(est, dataset, groundtruthtarget=None):
            myC = get_covariance(est)
            J = np.linalg.inv(myC)
            return impl.likelihood_set_fast(J, dataset)
...
def compute_scores(est, Xtrain, Xtest, cross_val=False,best_params=None):
    res = {}
...
class ConservativePCA(BaseEstimator):
    ...
    def score(self, X):
        ...
        return impl.likelihood_set_fast(J, X)
...
def  fit_Shrinkage_biasedmatrix_CV(Xtrain, Xtest, M0, cv_scoring="likelihood", n_jobs=None):
    ...
    return compute_scores(est, Xtrain, Xtest,best_params=cv.best_params_)
###########################################################
###########################################################
###########################################################
...
def fit_RIE_CV(Xtrain, Xtest, cv_scoring="likelihood", n_jobs=None):
    scoring = get_scoring_function(cv_scoring)
    #est = RIE().fit(Xtrain)
    T, N=np.shape(Xtrain)
...
class GradientAscent(BaseEstimator):
    ...
    def _fit(self, X):
        ...
        likelihoods_tr, likelihoods_te, completions_te, self.steps, J =\
            impl.gradient_ascent_ADAM_stop(Xtrain, Ctrain, self.maxepochs, self.eta, B, Jinit,\
    ...
    def _fit(self, X):
        ...
        if self.vectorconstraint:
            likelihoods_tr_GAW,likelihoods_te_GAW,completions_te_GAW,steps_GAW,J_GAW = \
                impl.gradient_ascent_Wishart_vectormultiplier(Xtrain,Ctrain,maxepochs=self.maxepochs,eta=self.eta,B=B,Y_init=Yinit,\
                                      bootstrapping=self.bootstrapping,datate=Xval,stop=self.stop)
        
        else:
            likelihoods_tr_GAW,likelihoods_te_GAW,completions_te_GAW,steps_GAW,J_GAW = \
                impl.gradient_ascent_Wishart_multiplier(Xtrain,Ctrain,maxepochs=self.maxepochs,eta=self.eta,B=B,Y_init=Yinit,\
                                      bootstrapping=self.bootstrapping,datate=Xval,stop=self.stop)
