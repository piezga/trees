...
def make_nutrient_grid(soil_binned, nutrient):
    """Convert binned soil data to 2D grid."""
...
# Stripped correlation (regress out nutrients)
...
# ============================================================
# Load Forest Map & Census Data for Raster Plots
# ============================================================
...
fig = plt.figure(figsize=(7.08, 7.08), dpi=300)
...
gs_top = gridspec.GridSpec(
    ...
    width_ratios=[1, 1, 1],  # col0 is wide, col1-2 are narrow
    height_ratios=[1, 1]
)
...
# Row 0: [a (wide)] [b (narrow left)] [empty?]
ax_a = fig.add_subplot(gs_top[0, 0])  # a - wide (col0)
ax_b = fig.add_subplot(gs_top[0, 1])  # b - narrow (col1)
# gs_top[0, 2] is empty

# Row 1: [c (wide)] [d (narrow left)] [empty?]
ax_c = fig.add_subplot(gs_top[1, 0])  # c - wide (col0) 
ax_d = fig.add_subplot(gs_top[1, 1])  # d - narrow (col1)
# gs_top[1, 2] is empty

# Bottom row: all narrow
ax_e = fig.add_subplot(gs_bot[0, 0])  # e - narrow
ax_f = fig.add_subplot(gs_bot[0, 1])  # f - narrow
ax_g = fig.add_subplot(gs_bot[0, 2])  # g - narrow
# ============================================================
# Panels a & b: Nutrient Heatmaps
# ============================================================

def plot_nutrient(ax, grid, label, cmap):
    im = ax.imshow(
...
plot_nutrient(ax_a, grid_N, 'N (g/kg)', cmap='YlGn')
plot_nutrient(ax_b, grid_pH, 'pH', cmap='RdYlBu')

ax_a.set_title('a', loc='left', fontweight='bold', fontsize=9)
ax_b.set_title('b', loc='left', fontweight='bold', fontsize=9)

# ============================================================
# Panel c: Load precomputed comparison image
# ============================================================

img_c = plt.imread(
    'stripped_correlation_analysis/scatter_raw_vs_stripped_20m.png')
ax_c.imshow(img_c, aspect='auto')
ax_c.axis('off')
ax_c.set_title('c', loc='left', fontweight='bold', fontsize=9)

# ============================================================
# Panel d: Delta heatmap (stripped - raw)
# ============================================================

vmax = np.nanpercentile(np.abs(delta), 95)
norm = mcolors.TwoSlopeNorm(vmin=-vmax, vcenter=0, vmax=vmax)

im_d = ax_d.imshow(delta, cmap=cmocean.cm.balance, norm=norm, rasterized=True)
cbar = plt.colorbar(im_d, ax=ax_d, fraction=0.046, pad=0.04)
cbar.ax.tick_params(labelsize=6)
cbar.set_label(r'$\Delta\rho$', fontsize=7)
ax_d.set_xlabel('Species', fontsize=7)
ax_d.set_ylabel('Species', fontsize=7)
...
def plot_species_pair(ax, pair, colors, label):
    """Plot two species over the BCI elevation map."""
    ax.imshow(
        bci_img[::-1], origin='lower', alpha=0.6,
        extent=[0, 1000, 0, 500], aspect='equal'
    )
    for idx, sp_idx in enumerate(pair):
        ...
        ax.scatter(
            sp_data['x'], sp_data['y'],
            s=0.2, color=colors[idx], alpha=0.6,
...
plot_species_pair(ax_e, pairs[0], species_colors[0], 'e')
plot_species_pair(ax_f, pairs[1], species_colors[1], 'f')
plot_species_pair(ax_g, pairs[2], species_colors[2], 'g')
...
plt.savefig('figures/Fig_nutrient_procedure.svg', dpi=300, bbox_inches='tight')
plt.show()
print("✓ Saved figures/Fig_nutrient_procedure.svg")
