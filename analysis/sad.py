...
for census in censi:
    ...
    if dataset.empty:
        ...
        dataset = dataset.join(temp_df, how='outer')
...
for kernel in chosen_kernels:
    ...
    for realization in range(1,num_realizations+1):
        ...
        current_file = senm_abundance_file_template.format(
            nx=nx,  
            ny=ny,
            nu=nu,
            kernel=kernel,
...
# Plot empirical data (dataset)
dataset.plot(logy=True, style='o', 
                  markersize=5, 
                  color=['blue', 'green'],  # Different colors for each census
                  ax=ax, 
                  label=['dataset Census 1', 'dataset Census 4'])
